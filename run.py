...
def runServer(adres=os.environ.get("ADDRESS"),port=os.environ.get("PORT")):
    ...
    return os.system("python3 manage.py runserver {0}:{1}" .format(adres,port))
def makeMigrations(params=""):
    print("{0} - Preparing migrations...".format(getCurrentTime()))
    return os.system("python3 manage.py makemigrations {0}" .format(params))
def collectStatic(params=None):
    print("{0} - Deploying statics...".format(getCurrentTime()))
    return os.system("python3 manage.py collectstatic {0}".format(params))
def migrate(params=""):
    print("{0} - Migrating to db...".format(getCurrentTime()))
    return os.system("python3 manage.py migrate {0}" .format(params))
def prints():
    print("="*40)
...
def tests(app=""):
    ...
    return os.system("python3 manage.py test {0}".format(app))
def cover(app=""):
    print("{0} - Covering tests...".format(getCurrentTime()))
    return os.system("coverage run manage.py test {0}".format(app))
def cover_raport():
    return os.system("coverage report")
def end_prints():
    print("="*30)
    print("Uruchom migracje poleceniem:")
    print("docker exec -it app sh -c '../initialize.sh'") 
    print("="*30)
if __name__ == "__main__":
    prints()
    if is_up(host=os.environ.get("DB_ADDRESS"),port=os.environ.get("DB_PORT"),count=3,sleep=1):
        print("Aktualna ścieżka: {0} ".format(os.system("pwd")))
        print("Pliki: {0} ".format(os.system("ls -lah")))
